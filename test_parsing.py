...
def test_extract_units_from_html_single_unit_multiple_subs():
    with open("test/html/single_unit_multiple_subs.html", "r") as f:
        ...
        assertEquals(units[0].video_youtube_url, 'https://youtube.com/watch?v=b7xgknqkQk8')
        assertEquals(units[0].mp4_urls[0], 'https://d2f1egay8yehza.cloudfront.net/edx-edx101/EDXSPCPJSP13-H010000_100.mp4')
        assertEquals(units[0].sub_template_url, 'https://courses.edx.org/courses/edX/DemoX.1/2014/xblock/i4x:;_;_edX;_DemoX.1;_video;_14459340170c476bb65f73a0a08a076f/handler/transcript/translation/%s')
        # log.info(units)

def test_extract_multiple_units_multiple_resources():
    with open("test/html/multiple_units.html", "r") as f:
        units = NewEdXPageExtractor().extract_units_from_html(f.read(), 'https://courses.edx.org')
        assertEquals(len(units), 3)
        # this one has multiple speeds in the data-streams field
        assertTrue('https://youtube.com/watch?v=CJ482b9r_0g' in [unit[0] for unit in units])
        assertTrue(len(units[0].mp4_urls) > 0)
        assertTrue('https://s3.amazonaws.com/berkeley-cs184x/videos/overview-motivation.mp4' in units[0].mp4_urls)
        assertEquals(units[0].resources_urls[0], 'https://courses.edx.org/static/content-berkeley-cs184x~2012_Fall/slides/overview.pdf')
...
def test_extract_courses_from_html():
    with open("test/html/dashboard.html", "r") as f:
        ...
        # log.info(len(courses))
        assertEquals(len(courses), 18)
        available_courses = [course for course in courses if course.state == 'Started']
        assertEquals(len(available_courses), 14)
